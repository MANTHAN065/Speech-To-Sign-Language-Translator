...
def filter_stop_words(words):
    stopwords_set = set(['a', 'an', 'the', 'is', 'am','be'])
...
#def stem_tokens(token_list):
 #   porter_stemmer = PorterStemmer()
  #  stemming_words = []
    ...
    #for token in token_list:
     #   stemming_words.append(porter_stemmer.stem(token))
...
def convert_eng_to_isl(input_string):
    ...
    print(possible_parse_tree_list);
...
def parseit():
    ...
    print("isling_parsed_token_list")
    print(isl_parsed_token_list)
    # print("isl_parsed_token_list: " + ' '.join(isl_parsed_token_list))

    # Stemming tokens
   # stem_isl_token_list = stem_tokens(isl_parsed_token_list)
